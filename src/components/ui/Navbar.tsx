  ...
  const handleSearch = async () => {
    try {
      ...
      const response = await fetch(
        `${url}/RememProt/get_gene_details/${searchQuery}`,
        {
          method: "GET",
          headers: {
      ...
      <nav className="w-full flex py-6 justify-between items-center navbar z-10">
        ...
        <ul className="list-none lg:flex hidden justify-center items-center flex-1">
          {navLinks.map((nav, index) => (
            <li
              key={nav.id}
              className={`font-poppins font-normal text-md cursor-pointer text-[17px] ${
                active === nav.id ? "text-blue-400" : "text-dimWhite"
        ...
        <Dialog>
          <div className="flex  justify-end items-center">
            ...
            <div
              className={`w-7 flex lg:hidden h-7  flex-col justify-between cursor-pointer transition-transform duration-300 ${
                toggle ? "open" : ""
              }`}
            ...
            >
              <div
                className={`bg-black rounded-5px w-full h-1 ${
                  toggle ? "hidden" : ""
                }`}
              ></div>
              <div
                className={`bg-black rounded-5px w-full h-1 transform transition-transform ${
                  toggle ? "rotate-45 -translate-x-1" : ""
                }`}
              ></div>
              <div
                className={`bg-black rounded-5px w-full h-1 transform transition-transform ${
                  toggle ? "-rotate-45 -translate-x-1 -translate-y-6" : ""
                }`}
              ></div>
            </div>
            ...
            {showGeneDetails && (
              <DialogContent className="w-full max-w-[80vh] max-h-[80vh] overflow-y-auto">
                <DialogHeader className="w-full">
                  <DialogTitle>
                    {" "}
                    <strong className="ms-2"> Gene Details :</strong>
                  </DialogTitle>
                ...
                {Object.keys(geneDetails).length > 0 ? (
                          ...
                          {Object.entries(geneDetails[geneSymbol]).map(
                            ([key, value], index) => (
                              <TableRow key={(index + 1)}>
                                <TableCell><strong>Result {index + 1}</strong>
                                  {typeof value === "object" ? (
                                    <Table className="mt-10 border-collapse border">
                                      <TableBody>
                  ...
                  <p className="text-red-500 text-center font-bold font-sans">
                    Gene not found. Please try a different search query.
                  </p>
                )}
              </DialogContent>
            )}
